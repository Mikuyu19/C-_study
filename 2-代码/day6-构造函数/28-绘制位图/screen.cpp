...
// 拷贝构造函数：
// 不能直接拷贝 rhs._fd 和 rhs._addr，因为它们是系统资源。
// 正确做法是复制普通成员，然后重新 open 和 mmap，得到属于新对象自己的资源。
Screen::Screen(const Screen& rhs)
    : _name(rhs._name), _fd(-1), _addr(0), _w(rhs._w), _h(rhs._h), _bpp(rhs._bpp)
{
    if (_name == 0)
    {
        return;
    ...
    printf("Screen copy constructor success!\n");
}
